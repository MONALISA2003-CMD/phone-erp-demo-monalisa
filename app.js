...
function applyPermissions(){
  ...
  // Agents and Shop Staff are individual contributors — their own
  // stock and aging is already shown right on their Dashboard, so the
  // standalone Inventory page (built for browsing/filtering across a
  // whole team or company) isn't something they need, per instruction.
  const navInventory=document.getElementById('nav-inventory');
  if(navInventory)navInventory.style.display=(u.role==='agent'||u.role==='shopstaff')?'none':'flex';
  ...
  // Reports and Performance pages are also company-wide analysis
  // tools that don't belong to individual contributors — Agent and
  // Shop Staff in particular should never see these.
  const reportsRestricted=['nav-reports'];
  reportsRestricted.forEach(id=>{
    const n=document.getElementById(id);
    if(n)n.style.display=['ceo','admin','regionalmanager','manager','teamleader','shopmanager'].includes(u.role)?'flex':'none';
  });
...
function go(id){
  ...
  // Reports: not available to individual contributors
  if(id==='reports'&&currentUser&&!['ceo','admin','regionalmanager','manager','teamleader','shopmanager'].includes(currentUser.role)){
    toast("You don't have access to this section",'var(--red)');
    return;
  }
  // Agents and Shop Staff use their Dashboard for stock, not the
  // standalone Inventory browsing page
  if(id==='inventory'&&currentUser&&(currentUser.role==='agent'||currentUser.role==='shopstaff')){
    toast("Your stock is on your Dashboard",'var(--amber)');
    go('dashboard');
    return;
  }
...
function openTeamStockDrilldown(teamLeaderUid,teamLeaderName){
  ...
  const blocks=tlAgents.map(a=>{
    const aStock=devices.filter(d=>d.holderUid===a.uid&&d.status!=='sold');
...
function renderRegionalManagerDashboard(el){
  ...
  const regionStock=devices.filter(d=>allUids.includes(d.holderUid)&&d.status!=='sold');
